...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['LUDSCALES_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['LUDSCALES_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Ludscales API.
 */
export class Ludscales {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['LUDSCALES_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('LUDSCALES_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.LudscalesError(
        "The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the Ludscales client with an apiKey option, like new Ludscales({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Ludscales.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('LUDSCALES_LOG'), "process.env['LUDSCALES_LOG']", this) ??
      defaultLogLevel;
  ...
  static Ludscales = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static LudscalesError = Errors.LudscalesError;
...
Ludscales.Pets = Pets;
Ludscales.Store = Store;
Ludscales.Users = Users;

export declare namespace Ludscales {
